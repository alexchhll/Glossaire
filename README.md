...
https://whatis.techtarget.com/fr/definition/Bootstrap
...
Aussi appelé event listeners.
Each available event has an event handler, which is a block of code (usually a JavaScript function that you as a programmer create) that will be run when the event fires. When such a block of code is defined to be run in response to an event firing, we say we are registering an event handler.
...
Hibernate, EclipseLink & consorts : des implémentations du standard JPA. En d’autres termes JPA constitue la théorie, et ces frameworks en sont la pratique.
...
Il y a une forte utilisation d’interfaces, et moins de sous-classes.
...
## Sérialisation
La sérialisation est le codage d'une information sous la forme d'une suite d'informations plus petites pour, par exemple, sa sauvegarde (persistance) ou son transport sur le réseau (proxy, RPC…) (objet vers string).
...
https://stackoverflow.com/questions/10179260/spring-framework-in-simple-terms
...
Un bon test est un test qui échoue.
Il permet de tester une fonction de l’application.
